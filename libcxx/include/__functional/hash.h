...
template <class _Data, class _Size>
inline _LIBCPP_INLINE_VISIBILITY
_Size
__loadword(const _Data* __p, _Size len)
{
    _Size __r = 0;
    // Find out how many _Data elements fits in _Size
    int byte_length = sizeof(_Size);
    if (byte_length > len)
        byte_length = len;
    int l = byte_length / sizeof(_Data);
    for(int i = 0; i < l; i++)
    {
        if (i)
            __r <<= sizeof(_Data)*8;
...
struct __murmur2_or_cityhash<_Size, 32>
{
    ...
    _Size operator()(const _Data* __key, _Size __len)
...
template <class _Data>
_Size
__murmur2_or_cityhash<_Size, 32>::operator()(const _Data* __key, _Size __len) _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
{
    const _Size __m = 0x5bd1e995;
    const _Size __r = 24;
    _Size __h = __len;
    const _Data* __data = __key;
    for (_Size i = 0; i < __len; __data += (4/sizeof(_Data)), i += 4)
    {
        _Size __k = __loadword<_Data, _Size>(__data, __len - i);
        __k *= __m;
        __k ^= __k >> __r;
        __k *= __m;
        __h *= __m;
