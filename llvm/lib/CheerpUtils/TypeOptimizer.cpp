...
TypeOptimizer::TypeMappingInfo TypeOptimizer::rewriteType(Type* t)
{
	...
	if(PointerType* pt=dyn_cast<PointerType>(t))
	{
		...
		if(newType->isArrayTy())
		{
			...
			return CacheAndReturn(PointerType::get(newType->getArrayElementType(), t->getPointerAddressSpace()), TypeMappingInfo::POINTER_FROM_ARRAY);
		}
		else if(newType == elementType)
			return CacheAndReturn(pt, TypeMappingInfo::IDENTICAL);
		else
			return CacheAndReturn(PointerType::get(newType, t->getPointerAddressSpace()), TypeMappingInfo::IDENTICAL);
	}
