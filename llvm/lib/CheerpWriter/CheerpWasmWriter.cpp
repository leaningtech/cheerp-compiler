...
void CheerpWasmWriter::encodeConstantDataVector(WasmBuffer& code, const llvm::ConstantDataVector* cdv)
{
	...
	{
		for (int i = 0; i < 4; i++)
			support::endian::write32le(bytes + i * 4, cdv->getElementAsInteger(i));
	}
	...
	{
		for (int i = 0; i < 16; i++)
			bytes[i] = cdv->getElementAsInteger(i);
	}
	else if (cdv->getElementType()->isIntegerTy(16))
	{
		for (int i = 0; i < 8; i++)
			support::endian::write16le(bytes + i * 8, cdv->getElementAsInteger(i));
	}
	else if (cdv->getElementType()->isFloatTy())
	{
		for (int i = 0; i < 4; i++)
		{
			...
			support::endian::write32le(bytes + i * 4, intReinterpret);
		}
	}
	else if (cdv->getElementType()->isDoubleTy())
	{
		for (int i = 0; i < 2; i++)
		{
			...
			support::endian::write64le(bytes + i * 2, intReinterpret);
		}
	}
	else
