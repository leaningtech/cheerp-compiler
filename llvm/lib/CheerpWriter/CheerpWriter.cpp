...
void CheerpWriter::compileEqualPointersComparison(const llvm::Value* lhs, const llvm::Value* rhs, CmpInst::Predicate p)
{
		...
		(rhsKind == REGULAR || rhsKind == SPLIT_REGULAR || rhsKind == RAW ||rhsKind == CONSTANT || (isGEP(rhs) && cast<User>(rhs)->getNumOperands()==2)))
	{
		...
		stream << joinString;
		compilePointerOffset(lhs, COMPARISON, /*forEscapingPointer*/false, /*useGPET*/true);
		stream << compareString;
		compilePointerOffset(rhs, COMPARISON, /*forEscapingPointer*/false, /*useGPET*/true);
	}
...
Type* CheerpWriter::getPointerElementTypeForValue(const Value* p, bool useGPET)
{
	...
	else if (const SelectInst* SI = dyn_cast<SelectInst>(p))
	{
		...
		for (unsigned int i = 1; i < SI->getNumOperands(); i++) {
			Type* T = getPointerElementTypeForValue(SI->getOperand(i), useGPET);
...
void CheerpWriter::compilePointerBase(const Value* p, bool forEscapingPointer, bool useGPET)
{
	...
	if (kind != RAW && (kind != CONSTANT || isa<ConstantPointerNull>(p)))
	{
		...
		compilePointerBaseTyped(p, getPointerElementTypeForValueAssert(p, useGPET), forEscapingPointer);
}
		...
		else if(tp->isPointerTy() && !TypeSupport::isRawPointer(tp, asmjsCalleeArg))
		{
				...
				(argKind == COMPLETE_OBJECT && curKind == BYTE_LAYOUT))
			{
				...
				else
				{
					compilePointerBase(*cur, true, true);
					stream << ',';
					compilePointerOffset(*cur, LOWEST, true, true);
				}
			}
...
void CheerpWriter::compileImports()
{
	...
	if(triple.isCheerpOS())
	{
		...
		if (WasmMappedMemory)
		{
			stream << "__mem_load_8:" << dummy << ',' << NewLine;
			stream << "__mem_load_16:" << dummy << ',' << NewLine;
			stream << "__mem_load_32:" << dummy << ',' << NewLine;
			stream << "__mem_load_64:" << dummy << ',' << NewLine;
			stream << "__mem_load_f:" << dummy << ',' << NewLine;
			stream << "__mem_load_d:" << dummy << ',' << NewLine;
			stream << "__mem_store_8:" << dummy << ',' << NewLine;
			stream << "__mem_store_16:" << dummy << ',' << NewLine;
			stream << "__mem_store_32:" << dummy << ',' << NewLine;
			stream << "__mem_store_64:" << dummy << ',' << NewLine;
			stream << "__mem_store_f:" << dummy << ',' << NewLine;
			stream << "__mem_store_d:" << dummy << ',' << NewLine;
		}
