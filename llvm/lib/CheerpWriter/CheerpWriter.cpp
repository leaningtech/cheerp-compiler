...
void CheerpWriter::compileMemFunc(const Value* dest, Type* destElementType, const Value* src, const Value* size)
{
	...
	{
		bool byteLayout = PA.getPointerKindAssert(dest) == BYTE_LAYOUT || TypeSupport::hasByteLayout(pointedType);
...
void CheerpWriter::compileAllocation(const DynamicAllocInfo & info)
{
	...
	bool isByteLayout = BYTE_LAYOUT == result || TypeSupport::hasByteLayout(t);
	...
	{
		if (info.useTypedArray() || isByteLayout)
		{
			stream << "(function(){";
			stream << "var __old__=";
	...
	else if (!info.sizeIsRuntime() )
	{
		...
		assert((REGULAR == result || SPLIT_REGULAR == result || isByteLayout) || numElem <= 1);
...
CheerpWriter::COMPILE_INSTRUCTION_FEEDBACK CheerpWriter::handleBuiltinCall(const CallBase& callV, const Function * func)
{
	...
	else if(ident=="cheerpCreate_ZN6client6StringC2EPU2jsKc")
	{
...
void CheerpWriter::compilePointerBaseTyped(const Value* p, Type* elementType, bool forEscapingPointer)
{
	...
	if(kind == CONSTANT)
	{
		if (isa<ConstantPointerNull>(p))
			stream << "nullArray";
		else if (isWasmTarget && !elementType->isStructTy())
		{
			compileHeapForType(elementType);
		}
		else
...
void CheerpWriter::compileByteLayoutOffset(const Value* p)
{
	// If the value has byte layout skip GEPS and BitCasts until the base is found
	while ( isBitCast(p) || isGEP(p) )
	{
		const User * u = cast<User>(p);
		Value* next = u->getOperand(0);
		bool byteLayoutFromHere = PA.getPointerKindAssert(next) != BYTE_LAYOUT;
		Type* curType = next->getType();
		if (isGEP(p))
		{
		...
		{
			compilePointerOffset(next, ADD_SUB);
			return;
		}
		p = next;
		continue;
...
CheerpWriter::COMPILE_INSTRUCTION_FEEDBACK CheerpWriter::compileNotInlineableInstruction(const Instruction& I, PARENT_PRIORITY parentPrio)
{
	...
	switch(I.getOpcode())
	{
		case Instruction::Alloca:
		{
			...
			if(k == BYTE_LAYOUT || TypeSupport::hasByteLayout(ai->getAllocatedType()))
			{
				assert(!allocaStores);
				stream << "{d:";
...
void CheerpWriter::compileGEPBase(const llvm::User* gep_inst, bool forEscapingPointer)
{
	...
	bool byteLayout = PA.getPointerKindAssert(gep_inst) == BYTE_LAYOUT || TypeSupport::hasByteLayout(elementTypeBaseOperand);
	if (byteLayout)
	{
		if (!TypeSupport::hasByteLayout(targetType) && forEscapingPointer)
		{
...
void CheerpWriter::compileGEPOffset(const llvm::User* gep_inst, PARENT_PRIORITY parentPrio)
{
	...
	bool byteLayout = PA.getPointerKindAssert(gep_inst) == BYTE_LAYOUT || TypeSupport::hasByteLayout(sourceType);
	if (byteLayout)
	{
		compileByteLayoutOffset(gep_inst);
	}
	else if (indices.size() == 1)
	{
...
void CheerpWriter::compileGlobal(const GlobalVariable& G)
{
	...
	if(G.hasInitializer())
	{
		...
		if(k == BYTE_LAYOUT || TypeSupport::hasByteLayout(G.getValueType()))
		{
			stream << "{d:";
