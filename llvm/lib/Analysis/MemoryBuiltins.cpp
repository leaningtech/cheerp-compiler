...
static Optional<AllocFnsTy> getAllocationSize(const Value *V,
  ...
  const Function *Callee =
      getCalledFunction(V, /*LookThroughBitCast=*/false, IsNoBuiltinCall, Caller);
  if (!Callee)
    return None;
...
static bool hasNoAliasAttr(const Value *V, bool LookThroughBitCast) {
  const auto *CB =
      dyn_cast<CallBase>(LookThroughBitCast ? V->stripPointerCastsSafe() : V);
...
bool llvm::isReallocLikeFn(const Function *F, const TargetLibraryInfo *TLI) {
  return getAllocationDataForFunction(F, ReallocLike, TLI, nullptr).hasValue();
}
...
const CallInst *llvm::isFreeCall(const Value *I, const TargetLibraryInfo *TLI) {
  ...
  const Function *Caller = nullptr;
  const Function *Callee =
      getCalledFunction(I, /*LookThroughBitCast=*/false, IsNoBuiltinCall, Caller);
