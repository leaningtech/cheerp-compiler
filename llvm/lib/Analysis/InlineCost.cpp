...
void InlineCostCallAnalyzer::updateThreshold(CallBase &Call, Function &Callee) {
  ...
  bool OnlyOneCallAndLocalLinkage = (DL.isByteAddressable() || F.getSection() == "asmjs") &&
      F.hasLocalLinkage() && F.hasOneLiveUse() && &F == Call.getCalledFunction();
...
InlineResult CallAnalyzer::analyze() {
  ...
  bool OnlyOneCallAndLocalLinkage = (DL.isByteAddressable() || F.getSection() == "asmjs") && F.hasLocalLinkage() && F.hasOneLiveUse() &&
