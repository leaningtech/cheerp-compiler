...
Instruction *MemCpyOptPass::tryMergingIntoMemset(Instruction *StartInst,
  ...
  for (const MemsetRange &Range : Ranges) {
    ...
    AMemSet = Builder.CreateMemSet(StartPtr, ByteVal, Range.End - Range.Start,
                                   MaybeAlign(Range.Alignment), false, NULL, NULL, NULL, IRBuilderBase::CheerpTypeInfo::get(DL.isByteAddressable(), nullptr));
    ...
    if (LI->isSimple() && LI->hasOneUse() &&
      ...
      if (C) {
        bool changed = performCallSlotOptzn(
            LI, SI, SI->getPointerOperand()->stripPointerCasts(DL.isByteAddressable()),
...
bool MemCpyOptPass::processMemCpy(MemCpyInst *M, BasicBlock::iterator &BBI) {
  ...
  const DataLayout &DL = M->getModule()->getDataLayout();
  if (auto *GV = dyn_cast<GlobalVariable>(M->getSource()->stripPointerCasts(DL.isByteAddressable())))
