...
private:
  ...
  void handleLoadOrStore(Type *Ty, Instruction &I, const APInt &Offset,
    ...
    // of bits" patterns.
    bool IsSplittable = DL.isByteAddressable() && Ty->isIntegerTy() && !IsVolatile;
  ...
  void visitPHINodeOrSelectInst(Instruction &I) {
    ...
    if (!DL.isByteAddressable() && I.getParent()->getParent()->getSection() != StringRef("asmjs")) {
      ...
      if(!firstUser || !isSafeToLoadUnconditionally(&I, I.getType()->getPointerElementType(), Align(1), DL, firstUser))
        return PI.setAborted(&I);
    }
