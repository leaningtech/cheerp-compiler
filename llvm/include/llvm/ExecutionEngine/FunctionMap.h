...
public:
	virtual llvm::Function* getFunctionOrNull(void* addr) = 0;
...
public:
	llvm::Function* getFunctionOrNull(void* addr) override
	{
		uintptr_t addri = reinterpret_cast<uintptr_t>(addr);
		auto it = rev_map.find(addri);
		return it != rev_map.end() ? it->second : nullptr;
	}
...
public:
	llvm::Function* getFunctionOrNull(void* addr) override
	{
		assert(false && "getFunctionOrNull not supported in DirectFunctionMap");
	}
	llvm::Function* getFunction(void* addr) override
	{
