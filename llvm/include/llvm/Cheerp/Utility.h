...
inline void assertPointerElementOrOpaque(llvm::Type* pointer, llvm::Type* pointee)
{
	...
	assert(llvm::cast<llvm::PointerType>(pointer)->isOpaque() || pointer->getNonOpaquePointerElementType() == pointee);
}
...
public:
	...
	static bool isCheerpOSFuncName(llvm::StringRef ident)
	{
		...
		return isWasiFuncName(ident) || ident.startswith("__dl_") || ident.startswith("__syscall_") || ident.startswith("__win_") || ident.startswith("__exc_") || ident.startswith("__jspi_");
	}
	...
	// Is this type an asmjs struct type?
	static bool isAsmJSPointed(const llvm::Type* t)
	{
		assert(t);
		while ( t->isArrayTy() )
