; RUN: opt -enable-gvn-sink=0 < %s -enable-coroutines -passes='default<O2>' -S | FileCheck --check-prefixes=CHECK %s
; RUN: opt -enable-gvn-sink=0 < %s -enable-coroutines -O0 -S | FileCheck --check-prefixes=CHECK-O0 %s
