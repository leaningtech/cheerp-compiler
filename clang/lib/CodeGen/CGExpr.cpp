...
Address CodeGenFunction::CreateIRTemp(QualType Ty, const Twine &Name) {
  ...
  unsigned AS = getContext().getCheerpTypeTargetAddressSpace(Ty, CurFn->getSection() == "asmjs");
...
void CodeGenFunction::EmitTypeCheck(TypeCheckKind TCK, SourceLocation Loc,
  ...
  auto PtrToAlloca = dyn_cast<llvm::AllocaInst>(Ptr->stripPointerCasts(true));
...
LValue CodeGenFunction::EmitCastLValue(const CastExpr *E) {
  ...
  case CK_ArrayToPointerDecay: {
    LValue LV = EmitLValue(E->getSubExpr());
    Address V = Builder.CreateConstArrayGEP(LV.getAddress(*this), 0);
