...
EmitArrayConstant(CodeGenModule &CGM, llvm::ArrayType *DesiredType,
  ...
  if (CGM.getTarget().isByteAddressable() && TrailingZeroes >= 8) {
        ...
        if (const RecordType* CurClass = CurType->getAs<RecordType>()) {
          ...
          if (const CXXRecordDecl *Base = dyn_cast<CXXRecordDecl>(BaseOrMember)) {
            ...
            if (CurClass->isUnionType()) {
              ...
              C = llvm::ConstantExpr::getBitCast(C, CGM.getTypes().ConvertTypeForMem(CurType, false, asmjs)->getPointerTo(AS));
