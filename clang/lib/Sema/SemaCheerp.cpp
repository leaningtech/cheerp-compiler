...
void cheerp::checkFunctionOnDeclaration(clang::FunctionDecl* FD, clang::Sema& sema, const bool isAlsoDefinition)
{
	...
	if (isClient)
	{
		...
		if (!isAlsoDefinition)
			TypeChecker::checkParameters<cheerp::TypeChecker::KindOfFunction::NamespaceClient, cheerp::TypeChecker::FailureMode::Diagnostic>(FD, sema);
	}
...
void cheerp::CheerpSemaData::checkFunctionToBeJsExported(const clang::FunctionDecl* FD, const bool isMethod, const bool isClient)
{
	...
	if (!isa<CXXConstructorDecl>(FD))
		TypeChecker::checkType<TypeChecker::KindOfValue::Return, TypeChecker::KindOfFunction::JSExported, TypeChecker::FailureMode::Diagnostic>(FD->getReturnType(), FD->getLocation(), sema, FD->getAttr<AsmJSAttr>());

	TypeChecker::checkParameters<TypeChecker::JSExported, TypeChecker::Diagnostic>(FD, sema);
