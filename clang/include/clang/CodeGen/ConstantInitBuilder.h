...
protected:
  ...
  llvm::Constant *finishStruct(llvm::StructType *structTy, llvm::StructType* directBase, bool asmjs, bool wasmgc);
...
public:
  ...
  void finishAndAddTo(AggregateBuilderBase &parent, llvm::StructType* directBase = nullptr, bool asmjs = false, bool wasmgc = false) {
    assert(this->Parent == &parent && "adding to non-parent builder");
    parent.add(asImpl().finishImpl(directBase, asmjs, wasmgc));
  }
  ...
  llvm::GlobalVariable *finishAndCreateGlobal(As &&...args) {
    ...
    return this->Builder.createGlobal(asImpl().finishImpl(nullptr, /*asmjs*/false, /*wasmgc*/false),
                                      std::forward<As>(args)...);
  }
  ...
  void finishAndSetAsInitializer(llvm::GlobalVariable *global, llvm::StructType* directBase = nullptr, bool asmjs = false, bool wasmgc = false) {
    assert(!this->Parent && "finishing non-root builder");
    return this->Builder.setGlobalInitializer(global, asImpl().finishImpl(directBase, asmjs, wasmgc));
  }
...
private:
  ...
  /// builder.
  llvm::Constant *finishImpl(llvm::StructType* directBase, bool asmjs, bool wasmgc) {
...
private:
  ...
  /// builder.
  llvm::Constant *finishImpl(llvm::StructType* directBase, bool asmjs, bool wasmgc) {
    return AggregateBuilderBase::finishStruct(StructTy, directBase, asmjs, wasmgc);
  }
};
