...
unsigned read8_1() {
  ...
  // SANITIZE: %bf.load = load i32, i32* bitcast {{.*}}, align 4
...
void write8_1() {
  ...
  // SANITIZE-NEXT: store i32 %bf.set, i32* bitcast {{.*}}, align 4
...
unsigned read16_1() {
  ...
  // CHECK: %bf.load = load i16, i16* bitcast (%struct.S2* @a2 to i16*), align 8
...
void write16_1() {
  ...
  // CHECK: store i16 5, i16* bitcast (%struct.S2* @a2 to i16*), align 8
  // CHECK-NEXT: ret void
