...
void for_array() {
  ...
  // CHECK: [[END]]:
  // CHECK: call void @_ZN1AD1Ev(%struct._Z1A* [[A]])
  // CHECK: ret void
}
...
void for_range() {
  ...
  for (B b : C()) {
    ...
    // CHECK: = call %struct._Z1B* @_Z5beginR1C(
    // CHECK: = call %struct._Z1B* @_Z3endR1C(
    // CHECK: br label %[[COND:.*]]
...
void for_member_range() {
  ...
  for (B b : D()) {
    ...
    // CHECK: = call %struct._Z1B* @_ZN1D5beginEv(
    // CHECK: = call %struct._Z1B* @_ZN1D3endEv(
    // CHECK: br label %[[COND:.*]]
