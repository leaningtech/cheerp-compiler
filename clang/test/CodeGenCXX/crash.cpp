// RUN: %clang_cc1 -triple %itanium_abi_triple %s -std=c++11 -emit-llvm-only
// RUN: %clang_cc1 -triple %itanium_abi_triple -emit-obj -o %t -debug-info-kind=line-tables-only -std=c++11 %s
// RUN: %clang_cc1 -triple %itanium_abi_triple -emit-obj -o %t -debug-info-kind=line-directives-only -std=c++11 %s
