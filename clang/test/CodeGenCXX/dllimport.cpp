...
USECLASS(W)
// vftable:
// MO1-DAG: @"??_SW@@6B@" = linkonce_odr unnamed_addr constant {{.*}} { [1 x ptr] [ptr @"?foo@W@@UAEXXZ"] }
// GO1-DAG: @_ZTV1W = available_externally dllimport unnamed_addr constant {{.*}} { [3 x ptr] [ptr null, ptr null, ptr @_ZN1W3fooEv] }
...
// G32-DAG: @_ZTV12KeyFuncClass = external dllimport unnamed_addr constant {{.*}}
...
namespace PR21355 {
  ...
  // GNU-DAG: @_ZTVN7PR213551SE = available_externally dllimport unnamed_addr constant {{.*}}
}
...
namespace PR27319 {
  ...
  void f() { new A<int>(); }
  // MO1-DAG: @"??_S?$A@H@PR27319@@6B@" = linkonce_odr unnamed_addr constant {{.*}}
}
