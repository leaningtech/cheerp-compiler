...
// CHECK-COMMON: @x = common {{(dso_local )?}}{{(addrspace\(.\) )?}}global
// CHECK-DEFAULT: @x = {{(dso_local )?}}{{(addrspace\(.\) )?}}global
int x;

// CHECK-COMMON: @ABC = {{(dso_local )?}}{{(addrspace\(.\) )?}}global
// CHECK-DEFAULT: @ABC = {{(dso_local )?}}{{(addrspace\(.\) )?}}global
typedef void* (*fn_t)(long a, long b, char *f, int c);
fn_t ABC __attribute__ ((nocommon));

// CHECK-COMMON: @y = common {{(dso_local )?}}{{(addrspace\(.\) )?}}global
// CHECK-DEFAULT: @y = common {{(dso_local )?}}{{(addrspace\(.\) )?}}global
