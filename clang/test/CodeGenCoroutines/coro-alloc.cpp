...
extern "C" void f0(global_new_delete_tag) {
  // CHECK: %[[ID:.+]] = call token @llvm.coro.id.p0i8.p0i8.p0i8(i32 16
  ...
  // CHECK: %[[FRAME:.+]] = call i8* @llvm.coro.begin.p0i8.p0i8(token %[[ID]], i8* %[[PHI]])

  // CHECK: %[[MEM:.+]] = call i8* @llvm.coro.free.p0i8.p0i8(token %[[ID]], i8* %[[FRAME]])
