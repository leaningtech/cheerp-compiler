...
// RUN: %clang -target x86_64-unknown-linux -ccc-print-phases -c %s -flto 2> %t
...
// RUN: %clang -target x86_64-unknown-linux -ccc-print-phases %s -flto 2> %t
...
// (unfortunately).
// RUN: %clang -target x86_64-unknown-linux %s -flto -save-temps -### 2> %t
...
// RUN: %clang -target x86_64-unknown-linux %s -flto -S -### 2> %t
// RUN: FileCheck -check-prefix=CHECK-COMPILE-SUFFIXES < %t %s
//
// CHECK-COMPILE-SUFFIXES: "-o" "{{.*}}lto.s" "-x" "c" "{{.*}}lto.c"

// RUN: not %clang -target x86_64-unknown-linux %s -emit-llvm 2>&1 | FileCheck --check-prefix=LLVM-LINK %s
// LLVM-LINK: -emit-llvm cannot be used when linking

/// With ld.bfd or gold, link against LLVMgold.
// RUN: %clang -target x86_64-unknown-linux -target x86_64-unknown-linux-gnu --sysroot %S/Inputs/basic_cross_linux_tree %s \
...
// RUN: %clang -target x86_64-unknown-linux -target x86_64-unknown-linux -### %s -flto -glldb 2> %t
// RUN: FileCheck -check-prefix=CHECK-TUNING-LLDB < %t %s
// RUN: %clang -target x86_64-unknown-linux -target x86_64-unknown-linux -### %s -flto -g 2> %t
