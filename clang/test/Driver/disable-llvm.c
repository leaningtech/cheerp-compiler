...
// RUN: %clang -target x86_64-unknown-linux -O2 -Xclang -disable-llvm-passes -### %s 2>&1 \
...
// RUN: %clang -target x86_64-unknown-linux -O2 -Xclang -disable-llvm-optzns -### %s 2>&1 \
// RUN:     | FileCheck --check-prefix=DISABLED-LEGACY %s
// RUN: %clang -target x86_64-unknown-linux -O2 -mllvm -disable-llvm-optzns -### %s 2>&1 \
// RUN:     | FileCheck --check-prefix=DISABLED-LEGACY %s
// DISABLED-LEGACY: -cc1
...
// RUN: %clang -target x86_64-unknown-linux -O2 -mllvm -disable-llvm-passes -### %s 2>&1 | FileCheck --check-prefix=MLLVM %s
// MLLVM: -cc1
