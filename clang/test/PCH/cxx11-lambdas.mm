// RUN: %clang_cc1 -triple %itanium_abi_triple -pedantic-errors -fblocks -std=c++1y -emit-pch %s -o %t-cxx11
// RUN: %clang_cc1 -triple %itanium_abi_triple -ast-print -pedantic-errors -fblocks -std=c++1y -include-pch %t-cxx11  %s | FileCheck -check-prefix=CHECK-PRINT %s
