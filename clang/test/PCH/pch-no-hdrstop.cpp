...
// RUN: %clang_cc1 -triple %itanium_abi_triple -verify -I %S -emit-pch -pch-through-hdrstop-create \
// RUN:   -fms-extensions -o %t.pch -x c++-header %s

// Create the PCH object
// RUN: %clang_cc1 -triple %itanium_abi_triple -verify -I %S -emit-obj -include-pch %t.pch \
