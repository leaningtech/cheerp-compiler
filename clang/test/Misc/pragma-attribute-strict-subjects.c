// RUN: %clang_cc1 -triple %itanium_abi_triple -fsyntax-only -Wno-pragma-clang-attribute -verify %s
// RUN: not %clang_cc1 -triple %itanium_abi_triple -fsyntax-only -ast-dump -ast-dump-filter test %s | FileCheck %s
